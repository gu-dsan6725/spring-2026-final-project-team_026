...
def main():
    parser = argparse.ArgumentParser(description='Selectively preprocess #ifdef / #ifndef statements.')
    parser.add_argument('-D', action='append', dest='defs', help='define name')
    parser.add_argument('-U', action='append', dest='undefs', help='undefine name')
    ...
    while 1:
        ...
        if keyword in ('ifdef', 'ifndef') and len(words) == 2:
            if keyword == 'ifdef':
                ...
                ko = 0
            word = words[1]
            if word in args.defs:
                stack.append((ok, ko, word))
                if not ko: ok = 0
            elif word in args.undefs:
